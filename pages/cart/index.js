...
const Cart = () => {
  ...
  /**
   * @TODO Change store module to seller module.
   * @TODO Reduce notification switch tab width.
   */

  return (
    <div className="grid grid-cols-1 md:grid-cols-5 mt-5 my-auto lg:px-10 p-3 md:gap-x-4 gap-y-4  lg:gap-x-8">
      <div className="col-span-3">
        <div className="flex  flex-col w-full rounded-md gap-y-3 shadow-md lg:p-8 p-5 border-secondary-300 bg-white">
          ...
          <div className="flex flex-col gap-y-3 h-[68vh] inset-y-0 my-auto overflow-y-auto overflow-x-hidden">
            {Array(10)
              .fill()
              .map((_, index) => (
                <div
                ...
                >
                  <div className="absolute transform rotate-45 font-medium right-0 -top-1 text-4xl ">
                    +
                  </div>
                  <div className="block">
                    <div className="relative h-[150px] w-[120px] flex">
                      <Image
                        src="/assets/images/house02.jpg"
                        alt="items added to cart"
                        layout="fill"
                        objectFit="cover"
                      />
                    </div>
                  ...
                  <div className="flex flex-col  gap-y-3">
                    <div className="flex flex-row font-bold gap-x-1.5 items-end">
                      ...
                      <p className="text-xs line-through mb-0.5">
                        ₦{NumberFormatter("7000")}
                      </p>
                    </div>
                    <p className="line-clamp-2">
                      Men's Leather Breathable Sports Casual Running Shoes -black
                    </p>
                    <div className="flex gap-x-5 items-center w-44">
                      <p className="flex flex-row items-center gap-x-1 text-xl rounded-md border border-secondary-300 h-7 place-content-center w-8">
                        -
                      </p>
                      <p className="flex flex-row items-center gap-x-1 text-xl rounded-md border border-secondary-300 h-7 place-content-center w-8">
                        +
        ...
        <div className="flex mx-auto flex-col rounded-md bg-white gap-y-3 w-full shadow-md lg:p-8 p-5 border-secondary-300">
          <div className="flex flex-col text-sm space-y-3">
            ...
            <select className="bg-inherit py-2 border-b rounded-sm border-gray-600 font-semibold text-secondary-400">
              {["Standard Delivery (Free)", "Fast Delivery + $50"].map(
                (option, index) => (
                  <option
                    key={index}
                    className=" animate-select border-b-2 border-gray-700"
                  >
                    {option}
                  </option>
                )
            ...
            <button className="w-full rounded-sm lg:text-base text-sm bg-green-600 font-semibold text-center py-3 text-secondary-50">
              CHECKOUT
            </button>
