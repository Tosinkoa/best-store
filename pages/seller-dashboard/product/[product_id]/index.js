    ...
    <div className="flex flex-col mx-auto w-[95%] md:h-[80vh] mt-4">
      <div className="grid md:grid-cols-2 md:gap-x-7 gap-y-4 w-full h-full">
        <div className="flex flex-row md:gap-x-4 gap-x-2  ">
          <div className="w-24 md:h-[60vh] h-[50vh] space-y-2 overflow-y-auto">
            {Array(10)
              .fill()
              .map((_, index) => (
                <div key={index} className="relative h-24 w-full rounded-sm">
                  <Image src="/assets/images/house07.jpg" alt="product small image" layout="fill" objectFit="cover" className="rounded-sm" />
                </div>
              ))}
          </div>
          <div className="relative md:h-[60vh] h-[50vh] w-full rounded-sm">
            <Image src="/assets/images/house07.jpg" alt="product-image" layout="fill" objectFit="cover" className="rounded-sm" />
          </div>
        </div>
        <div className="w-full flex flex-col gap-y-3  ">
          <div className="relative flex flex-row gap-x-2 ">
            <p className="font-semibold text-lg md:text-2xl">Men's Canvas Breathable Sports Casual Running Shoes -blue</p>
          ...
          <div className="border border-secondary-400 rounded-sm py-1 px-2 font-semibold w-fit">6 in stock</div>
          <hr />
          <div className="flex gap-x-4 items-center">
            ...
            <AiOutlineMinus onClick={() => setItemsToAdd((prev) => prev - 1)} className="border border-secondary-400 rounded-sm py-1 px-2 text-2xl font-semibold w-fit" />
            <AiOutlinePlus onClick={() => setItemsToAdd((prev) => prev + 1)} className="border border-secondary-400 rounded-sm py1 px-2 text-2xl font-semibold w-fit" />
            <button className="py-2 text-secondary-50 font-semibold px-3 bg-primary-600 text-xs rounded-sm">ADD TO CART</button>
          </div>
      ...
      <div className="flex flex-col">
        ...
        <div className="flex-row flex gap-x-3 overflow-x-auto">
          {Array(10)
            ...
            .map((_, index) => (
              <div className="block">
                <div className="relative h-32 w-32 rounded-sm">
                  <Image src="/assets/images/house01.jpg" alt="product" layout="fill" objectFit="cover" className="rounded-sm" />
                </div>
              </div>
            ))}
        </div>
